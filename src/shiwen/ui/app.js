...
(() => {
  ...
  const words = {
    zh: {
      ...
      reveal: "在文件夹中显示",
      close: "关闭",
      chooseDocument: "选择一份文档，查看相关段落。",
      previous: "上一处命中",
      next: "下一处命中",
      openOriginal: "打开原文件",
      privacyFooter: "文件留在本机，内容只属于你",
      noAccount: "无账号 · 无上传 · 无遥测",
      language: "语言",
      appearance: "外观",
      system: "跟随系统",
      light: "浅色",
      ...
      standard: "标准",
      low: "低占用",
      resourceNote: "后台串行解析文档。低占用模式会在任务之间留出更多间隔。",
      localData: "本地索引占用",
      indexPrivacy:
      ...
      partial: "部分页面无可提取文字",
      no_text: "未提取到文字，可能需要 OCR",
      encrypted: "文件受密码保护",
      file_limit: "超过 100 MiB 文件上限",
      text_limit: "超过文本提取上限",
      page_limit: "超过 1,000 页上限",
      timeout: "解析超时",
      parse_error: "未能解析文件",
      encoding: "请将文本另存为 UTF-8",
      permission: "没有读取权限",
      cloud: "文件尚未下载到本机",
      pending: "内容待更新",
      storage_error: "本地存储异常，请检查磁盘空间",
      query_too_short: "每个搜索词至少需要 2 个字符",
      query_too_long: "搜索内容不能超过 256 个字符",
      too_many_terms: "最多组合 12 个搜索词",
      unclosed_quote: "请补全短语的双引号",
      invalid_folder: "文件夹无效或不可访问",
      folder_too_broad:
        "请选择具体的文档目录，不要选择整盘或包含拾文数据的目录",
      invalid_exclusion: "请输入当前范围内的有效子目录",
      invalid_setting: "设置值无效",
      operation_failed: "操作未完成，请检查文件与权限后重试",
      desktop_only:
        "请在桌面应用中使用系统文件夹选择器；开发预览可通过 --folder 添加目录",
      loading: "正在搜索…",
      disconnected: "目录不可用",
    ...
    en: {
      ...
      close: "Close",
      chooseDocument: "Select a document to read the matching passage.",
      previous: "Previous match",
      next: "Next match",
      openOriginal: "Open original",
      privacyFooter: "Your files stay here. Your words stay yours.",
      noAccount: "No account · No uploads · No telemetry",
      language: "Language",
      ...
      dark: "Dark",
      glass: "Frosted glass",
      resource: "Background usage",
      standard: "Standard",
      low: "Low",
      resourceNote:
        "Documents are parsed one at a time. Low mode adds a pause between tasks.",
      localData: "Local index size",
      indexPrivacy:
      ...
      nameOnly: "Name match",
      savedToast: "Bookmark added",
      unsavedToast: "Bookmark removed",
      page: "Page {n}",
      line: "From line {n}",
      paragraph: "Paragraph {n}",
      match: "Match location {n} of {total}",
      block: "Content {n} of {total}",
      textPreview: "Extracted text preview",
      noTextPreview:
        "There is no text to preview. Open the original file, or check its indexing status.",
      removed: "Search scope removed. Original files are unchanged.",
      remove: "Remove",
      exclude: "Exclude subfolder",
      excluded: "Excluded: ",
      removeConfirm:
        "Remove this folder’s index and bookmark references? Original files will not be deleted.",
      excludePrompt:
      ...
      file_limit: "File exceeds 100 MiB",
      text_limit: "Extracted text exceeds the limit",
      page_limit: "PDF exceeds 1,000 pages",
      timeout: "Parsing timed out",
      parse_error: "Could not parse this file",
      encoding: "Please save this text file as UTF-8",
      permission: "Read permission denied",
      cloud: "File has not been downloaded",
      pending: "Content update pending",
      storage_error: "Local storage error; check available disk space",
      query_too_short: "Each search term needs at least 2 characters",
      query_too_long: "Search text is limited to 256 characters",
      too_many_terms: "Use up to 12 search terms",
      ...
      operation_failed:
        "Could not complete the operation. Check the file and permissions, then retry.",
      desktop_only:
        "Use the folder picker in the desktop app. In developer preview, add folders with --folder.",
      loading: "Searching…",
      disconnected: "Folder unavailable",
      partialList: "First 50 results; load more to continue",
    },
  };
  function t(key, values = {}) {
    let text = words[state.language][key] || words.en[key] || key;
  ...
  const paths = {
    ...
    up: "M6 15l6-6 6 6",
    down: "M6 9l6 6 6-6",
    external: "M13 4h7v7M20 4l-10 10M10 4H4v16h16v-6",
  ...
  function icon(name) {
    ...
    path.setAttribute("d", paths[name] || paths.files);
    svg.append(path);
    return svg;
  }
  ...
  function highlight(node, text, spans = []) {
    const chars = Array.from(text);
    node.replaceChildren();
    let previous = 0;
    for (const [a, b] of spans) {
      node.append(document.createTextNode(chars.slice(previous, a).join("")));
      node.append(make("mark", "", chars.slice(a, b).join("")));
      previous = b;
    }
  ...
  async function api(method, args = {}) {
    ...
    if (window.pywebview?.api)
      result = await window.pywebview.api.call(method, args);
    else if (window.SHIWEN_BOOT.token) {
      const response = await fetch("/api", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-Shiwen-Token": window.SHIWEN_BOOT.token,
        },
  ...
  function applySettings(settings) {
    ...
    document.body.classList.toggle("solid", !settings.glass);
  ...
  function drawFolders() {
    ...
    $("folder-management").replaceChildren();
    for (const root of state.status.roots) {
      const button = make(
        "button",
        `folder-item${state.root === root.id ? " active" : ""}`,
      );
      button.append(
        icon("folder"),
      ...
      button.onclick = () => {
        state.root = state.root === root.id ? null : root.id;
        state.view = "all";
        updateNavigation();
        drawFolders();
        run(() => search());
      ...
      exclude.onclick = () =>
        run(async () => {
          const answer = await confirm(t("exclude"), t("excludePrompt"), "");
          if (answer === null) return;
          await api("exclude", { root_id: root.id, relative: answer });
          await refreshStatus();
          await search();
        });
      remove.onclick = () =>
        run(async () => {
          if ((await confirm(t("remove"), t("removeConfirm"))) === null) return;
          await api("remove_root", { root_id: root.id });
          if (state.root === root.id) state.root = null;
          state.selected = null;
          await refreshStatus();
          await search();
          toast(t("removed"));
  ...
  function updateNavigation() {
    document.querySelectorAll("[data-view]").forEach((b) => {
      const active = b.dataset.view === state.view && !state.root;
  ...
  async function refreshStatus() {
    ...
    const processing = t("processed", {
      n: status.progress.discovered,
    ...
    for (const [name, n] of Object.entries(status.counts.by_status)) {
      ...
      row.append(make("span", "", t(name)), make("span", "", n));
      $("status-counts").append(row);
    }
    $("welcome").hidden = Boolean(status.roots.length);
    $("workspace").hidden = !status.roots.length;
    if (
      JSON.stringify(previous?.roots) !== JSON.stringify(status.roots) ||
      previous?.settings.language !== status.settings.language
    )
      drawFolders();
    return (
      !previous ||
      JSON.stringify(previous.counts) !== JSON.stringify(status.counts) ||
      previous.progress.processed !== status.progress.processed
    );
  }
  async function search(append = false) {
    const sequence = ++state.searchSequence;
    const offset = append ? state.items.length : 0;
    $("result-summary").textContent = t("loading");
    try {
      ...
      $("result-summary").textContent = t(
        result.has_more ? "atLeast" : "found",
      ...
      if (!state.items.some((item) => item.id === state.selected)) {
        ...
        await select(state.selected, false);
      } else await select(state.selected, false);
    } catch (error) {
      if (sequence !== state.searchSequence) return;
      $("result-summary").textContent = t(error.message);
      state.items = [];
  ...
  function drawResults(message) {
    ...
    if (!state.items.length) {
      ...
      empty.append(
        ...
        make(
          "div",
          "",
          message || t(state.view === "saved" ? "savedEmpty" : "noResults"),
        ),
        make("p", "", t("noResultsHint")),
      );
    ...
    for (const item of state.items) {
      const button = make(
        "button",
        `result${item.id === state.selected ? " active" : ""}`,
      );
      button.dataset.id = item.id;
      button.setAttribute("aria-pressed", String(item.id === state.selected));
      const head = make("span", "result-head"),
        file = make(
          "span",
          `file-icon ${item.type}`,
          item.type === "folder"
      ...
      highlight(title, item.name, item.title_ranges);
      const date = new Date(item.mtime_ns / 1e6).toLocaleDateString(
      ...
      labels.append(
        ...
        make(
          "span",
          "result-meta",
          `${date} · ${size(item.size)}${item.saved ? " · " + t("saved") : ""}`,
        ),
      );
      ...
      for (const excerpt of item.excerpts) {
        ...
        if (excerpt.after) span.append(document.createTextNode("…"));
        button.append(span);
      }
      ...
      button.onclick = () => run(() => select(item.id, true));
      button.ondblclick = () =>
        run(() => api("open", { document_id: item.id }));
      button.onkeydown = (event) => {
        if (["ArrowDown", "ArrowUp"].includes(event.key)) {
          event.preventDefault();
          const index = state.items.findIndex((i) => i.id === item.id);
          const next =
            state.items[index + (event.key === "ArrowDown" ? 1 : -1)];
          if (next) {
            run(() => select(next.id, false));
            $("results").querySelector(`[data-id="${next.id}"]`)?.focus();
          }
  ...
  async function select(id, show = true) {
    state.selected = id;
    ...
    state.hits = document.blocks
      .map((b, i) => (b.ranges.length ? i : -1))
      .filter((i) => i >= 0);
    state.block = state.hits[0] ?? 0;
  ...
  function drawBlock() {
    ...
    if (!doc) return;
    const block = doc.blocks[state.block];
    const indices = state.hits.length
      ? state.hits
      : doc.blocks.map((_, i) => i);
    const position = indices.indexOf(state.block);
    $("match-label").textContent = t(state.hits.length ? "match" : "block", {
      n: indices.length ? position + 1 : 0,
      total: indices.length,
    });
    $("prev-hit").disabled = position <= 0;
    $("next-hit").disabled = position >= indices.length - 1;
    $("paper-location").textContent = block
      ? t(block.kind, { n: block.location })
      : "";
  ...
  function moveHit(step) {
    ...
    const indices = state.hits.length
      ? state.hits
      : state.document.blocks.map((_, i) => i);
    const next = indices[indices.indexOf(state.block) + step];
    if (next !== undefined) {
      state.block = next;
      drawBlock();
    }
  }
  function confirm(title, text, value = null) {
    $("confirm-title").textContent = title;
    $("confirm-text").textContent = text;
    $("confirm-input").hidden = value === null;
    ...
    return new Promise((resolve) => {
      ...
      $("confirm-ok").onclick = () =>
        finish(value === null ? true : $("confirm-input").value);
      $("confirm-cancel").onclick = () => finish(null);
      $("confirm-dialog").oncancel = () => finish(null);
    });
  }
  async function addFolder() {
    const id = await api("choose_folder");
    if (id !== null) {
      await refreshStatus();
      await search();
    }
  ...
  document.querySelectorAll("[data-icon]").forEach((el) => {
    if (el.className) el.append(icon(el.dataset.icon));
    else el.replaceWith(icon(el.dataset.icon));
  });
  ...
  $("query").oninput = schedule;
  $("query").oncompositionstart = () => {
    state.composing = true;
    clearTimeout(search.timer);
  };
  $("query").oncompositionend = () => {
  ...
  $("load-more").onclick = () => run(() => search(true));
  $("bookmark").onclick = () =>
    run(async () => {
      const saved = !state.document.saved;
      await api("bookmark", { document_id: state.selected, saved });
      await refreshStatus();
      await search();
      toast(t(saved ? "savedToast" : "unsavedToast"));
    });
  ...
  $("next-hit").onclick = () => moveHit(1);
  $("close-preview").onclick = () =>
    $("workspace").classList.remove("show-preview");
  $("pause").onclick = () =>
    run(async () => {
      await api("pause", { paused: !state.status.paused });
      await refreshStatus();
    });
  $("retry").onclick = () =>
    run(async () => {
  ...
  for (const key of ["theme", "language", "glass", "resource"])
    $(key).onchange = () =>
      run(async () => {
        await api("setting", {
          key,
          value: key === "glass" ? $(key).checked : $(key).value,
        });
        await refreshStatus();
        drawFolders();
        await search();
  ...
  $("clear-data").onclick = () =>
    run(async () => {
      if ((await confirm(t("clearData"), t("clearConfirm"))) === null) return;
      await api("clear");
      state.selected = null;
      state.root = null;
      state.query = "";
  ...
  matchMedia("(prefers-color-scheme: dark)").addEventListener("change", () => {
    if (state.status) applySettings(state.status.settings);
  });
  ...
  if (window.SHIWEN_BOOT.token) start();
})();
