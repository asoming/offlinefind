...
- 无边框窗口，最小化 / 最大化 / 关闭融入界面，支持拖动与边缘缩放。
...
[新版基准](scripts/acceptance_benchmark.py)覆盖一万份混合文档、100 条查询与百万条合成名称；本机完整桌面空闲 RSS 约 **574 MiB**，部分正文查询约 **0.4–0.8 秒**，百万名称的大量匹配查询约 **2.7 秒**。三平台均运行原生桌面自动检查。测量方法、原始报告及延期项见[性能报告](docs/PERFORMANCE.zh-CN.md)，不宣称达到原定低内存及全部查询延迟目标。后续见[路线图](docs/ROADMAP.md)。
