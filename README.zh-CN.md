...
**当前版本：v0.1.0-beta.4。** 这是公开测试版，不代表 PRD 全部功能已完成。请先阅读[版本说明与限制](docs/RELEASE_NOTES.md)。
...
浏览器模式只监听本机回环地址，带每次启动生成的请求令牌，不是云端服务。`--folder` 仅用于开发时限制测试范围；正常启动不需要此参数。系统应用打开功能以桌面版为准。运行时可离线；源码安装依赖需联网或事先准备包缓存。
