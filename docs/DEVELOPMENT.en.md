...
`tests/test_shutdown.py` covers callback results/errors, lost callbacks, real parser cancellation and restart, and active RPC/database shutdown ordering. `--gui-close-smoke RESULT_JSON` closes a native window while JavaScript is still executing; the subprocess must exit successfully within 15 seconds. Linux packaging and installed DEB checks on Ubuntu 22.04 / 24.04 run this regression in addition to the normal GUI smoke test.
